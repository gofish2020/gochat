...
func (rc *RpcConnectClient) GetRpcClientByServerId(serverId string) (c client.XClient, err error) {
	...
	// 例如：serverId下面有3个服务；每次连接服务的时候，为了保证均衡性，第一个连接serverId的第一个，第二个连接第二个 等等
	if _, ok := rc.IndexMap[serverId]; !ok {
		rc.IndexMap = map[string]int{ // rc.IndexMap 用来记录用了serverId下的第几个服务
			serverId: 0,
		}
	}

	// 基于 rc.IndexMap 中保存的值，计算应该使用serverId下的哪个索引idx 实例
	idx := rc.IndexMap[serverId] % len(rc.ServerInsMap[serverId])
	ins := rc.ServerInsMap[serverId][idx]
	// rc.IndexMap 中的值每次用完之后 +1（实现了轮训的效果），用于下次直接使用
...
func (rc *RpcConnectClient) GetAllConnectTypeRpcClient() (rpcClientList []client.XClient) {
	for serverId, _ := range rc.ServerInsMap { // rc.ServerInsMap 中有多个serverid，
		c, err := rc.GetRpcClientByServerId(serverId) // 从每个 serverid 中选出一个rpc client
		if err != nil {
			logrus.Infof("GetAllConnectTypeRpcClient err:%s", err.Error())
			continue
		}
		rpcClientList = append(rpcClientList, c) // 组成一个集合
	}
	return
}
...
func (task *Task) pushSingleToConnect(serverId string, userId int, msg []byte) {
	...
	// 发送rpc请求，调用connect服务中的PushSingleMsg函数，代码位于 connect/rpc.go
