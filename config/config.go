...
// 获取函数getCurrentDir所在的目录：即 config目录
...
func Init() {
	once.Do(func() {
		...
		configFilePath := realPath + "/" + env + "/" // config/dev/

		viper.SetConfigType("toml")         // 配置文件后缀
		viper.SetConfigName("/connect")     // 配置文件名
		viper.AddConfigPath(configFilePath) // 配置文件所在的目录
		err := viper.ReadInConfig()         // viper读取toml文件内容
		if err != nil {
			panic(err)
		}
		viper.SetConfigName("/common")
		err = viper.MergeInConfig() // 将toml文件内容全都保存到viper缓存中
		if err != nil {
			panic(err)
		}
